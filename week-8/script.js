...
(function () {
    ...
    function applyFilters() {
        ...
        let minAttendance = minAttendanceFilter && minAttendanceFilter.value !== ''
            ? Number(minAttendanceFilter.value)
            : null;
        let maxAttendance = maxAttendanceFilter && maxAttendanceFilter.value !== ''
            ? Number(maxAttendanceFilter.value)
            : null;

        // a min greater than max isn't a real filter, it's a mistake — flag it
        // clearly instead of silently showing zero confusing results
        const rangeErrorEl = document.getElementById('attendanceRangeError');
        if (rangeErrorEl) {
            if (minAttendance !== null && maxAttendance !== null && minAttendance > maxAttendance) {
                rangeErrorEl.textContent = 'Min attendance can\'t be greater than max — ignoring both for now.';
                minAttendance = null;
                maxAttendance = null;
            } else {
...
(function () {
    ...
    const colorRisk = rootStyles.getPropertyValue('--danger').trim() || '#c0392b';
